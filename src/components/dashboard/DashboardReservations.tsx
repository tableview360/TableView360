...
export default function DashboardReservations() {
  ...
  async function load() {
    ...
    let query = supabase
      .from('reservations')
      .select('*, profiles!reservations_client_id_fkey(full_name)')
        ...
        {['all', 'pending', 'confirmed', 'cancelled'].map((f) => (
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-medium transition ${
              filter === f ? 'bg-emerald-600 text-white' : 'bg-slate-800/80 text-slate-400 border border-slate-700/40 hover:bg-slate-800/50'
            }`}
          >
            ...
            <tbody>
              {reservations.map((r) => (
                <tr key={r.id} className="border-b border-slate-800 text-sm">
                  ...
                  <td className="px-6 py-3 text-slate-400 max-w-[200px] truncate">{r.notes || '—'}</td>
